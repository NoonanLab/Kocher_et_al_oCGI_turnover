...
setwd('/gpfs/gibbs/pi/noonan/ak2267/speciesPairs/CGIcentric/permutation_diffReq')
...
# store difference requirement for this permutation
diffReq <- as.numeric(args[2])
...
# add information on OE ratio difference
table$OE_A <- (table$NumCpGs_ReconciledCGI_A * table$lengthCGI_A) / (table$NumC_A * table$NumG_A)
table$OE_B <- (table$NumCpGs_ReconciledCGI_B * table$lengthCGI_B) / (table$NumC_B * table$NumG_B)

if (min(table$NumC_A==0)) {
  table[table$NumC_A==0,]$OE_A <- 0
}
if (min(table$NumG_A==0)) {
  table[table$NumG_A==0,]$OE_A <- 0
}
if (min(table$NumC_B==0)) {
  table[table$NumC_B==0,]$OE_B <- 0
}
if (min(table$NumG_B==0)) {
  table[table$NumG_B==0,]$OE_B <- 0
}

table$OE_diff <- 1
table[table$CGI_summary=='A',]$OE_diff <- table[table$CGI_summary=='A',]$OE_A - table[table$CGI_summary=='A',]$OE_B
table[table$CGI_summary=='B',]$OE_diff <- table[table$CGI_summary=='B',]$OE_B - table[table$CGI_summary=='B',]$OE_A

# restrict table based on OE ratio difference using cutoff read in as an argument
table <- table[table$OE_diff >= diffReq,]
...
write.table(t(data.frame(c(infoList, FD_list, p_list, args[2]))), 
            file = paste(label, '_', dimensions, 'x3_', args[2], '.txt', sep = ''), 
