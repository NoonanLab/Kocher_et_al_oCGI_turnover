...
# Modified 3/7/24 to do getFasta and faCount on ORIGINAL CGI tracks - not reconciled
# This will make comparisons in Fig 2C and related supplement more accurate for Obs/Exp ratio
...
for speciesCombo in SpeciesCombos:
    for tissue in Tissues:
        for mark in Marks:
            ...
            ageSegmentationHuman = '/home/ak2267/genomes/EnhancerAge/clade_specific_elements_hg38.bed'
            ...
            outputString.append('mkdir Roller_'+speciesCombo+'_'+tissue+'_'+mark+'_OriginalGCcounts ; cd Roller_'+speciesCombo+'_'+tissue+'_'+mark +'_OriginalGCcounts')
            ...
            # NEW IN THIS SCRIPT
            # use just created intersections with original CGIs to make a bed file to use with getFasta and faCount later in the script
            outputString.append('awk \'{ print $5\"\\t\"$6\"\\t\"$7\"\\t\"$4 }\' speciesA_reconciledCGI_intersectSpeciesA_CGI.txt | grep -v \'\\.\' > speciesA_originalCGIs.bed')
            outputString.append('awk \'{ print $5\"\\t\"$6\"\\t\"$7\"\\t\"$4 }\' speciesB_reconciledCGI_intersectSpeciesB_CGI.txt | grep -v \'\\.\' > speciesB_originalCGIs.bed')
            ...
            # lift back to other species and keep sites in human that lift to the same peak as the one they started as (analagous to pipeline in consensusCGIs)
            ...
            # lift back to human and restrict list in speciesA and speciesB to only those that lift back to the same CGI in human (analogous to above)
            ...
            outputString.append('module load Subread/2.0.3-GCC-10.2.0')
            ...
            # NEW IN THIS SCRIPT 
            # STEP 7: run bedtools getfasta and faCount to get CpG numbers in ORIGINAL CGIs, and in reconciled Peaks
            outputString.append('bedtools getfasta -name -fi /gpfs/gibbs/pi/noonan/ak2267/genomes/'+speciesA+'.fa.masked -bed speciesA_originalCGIs.bed > speciesA_reconciledCGI.fa ; faCount speciesA_reconciledCGI.fa > speciesA_reconciledCGI.faCount')
            outputString.append('bedtools getfasta -name -fi /gpfs/gibbs/pi/noonan/ak2267/genomes/'+speciesB+'.fa.masked -bed speciesB_originalCGIs.bed > speciesB_reconciledCGI.fa ; faCount speciesB_reconciledCGI.fa > speciesB_reconciledCGI.faCount')
            outputString.append('bedtools getfasta -name -fi /gpfs/gibbs/pi/noonan/ak2267/genomes/'+speciesA+'.fa.masked -bed speciesA_reconciledPeaks.bed > speciesA_reconciledPeaks.fa ; faCount speciesA_reconciledPeaks.fa > speciesA_reconciledPeaks.faCount')
            outputString.append('bedtools getfasta -name -fi /gpfs/gibbs/pi/noonan/ak2267/genomes/'+speciesB+'.fa.masked -bed speciesB_reconciledPeaks.bed > speciesB_reconciledPeaks.fa ; faCount speciesB_reconciledPeaks.fa > speciesB_reconciledPeaks.faCount')
            ...
            outputString.append('python /gpfs/gibbs/pi/noonan/ak2267/speciesPairs/Scripts/ak20240307_countFinalOverlaps_speciesPairs_CGIcentric_addGCcounts.py '+' '.join(pythonInputFiles)+' >> '+workingDir+'/SummaryFiles_noPromFilter_OriginalGCcounts/'+speciesCombo+'_'+tissue+'_'+mark+'.txt')
