            ...
            for timePointIndex in [0,1,2,3]:
                ...
                if timePointA != '.' and timePointB != '.':
                    ...
                    if speciesA == 'hg19':
                    	...
                    	featuresArrayB.append(pathToBlacklist+'blacklist_'+speciesA+'.bed')
                    if speciesB == 'mm9':
                    	featuresArrayB.append(pathToFANTOM+'FANTOM_TSS_'+speciesB+'.bed')
                    	featuresArrayB.append(pathToBlacklist+'blacklist_'+speciesB+'.bed')

                    outputString.append('bedtools intersect -wa -u -a '+pathToPeaksA+' -b '+' -b '.join(featuresArrayA)+' | bedtools intersect -v -a '+reconciledA+' -b - > speciesA_intermediate_CGIsnoPromPeak.bed')
                    outputString.append('bedtools intersect -wa -u -a '+pathToPeaksB+' -b '+' -b '.join(featuresArrayB)+' | bedtools intersect -v -a '+reconciledB+' -b - > speciesB_intermediate_CGIsnoPromPeak.bed')
                    ...
                    outputString.append('python /gpfs/gibbs/pi/noonan/ak2267/speciesPairs/Scripts/countFinalOverlaps_speciesPairs_CGIcentric.py '+' '.join(pythonInputFiles)+' >> '+workingDir+'/Noonan_summaryFiles/'+speciesCombo+'_'+tissue+'_'+mark+'_'+str(timePointIndex)+'.txt')
