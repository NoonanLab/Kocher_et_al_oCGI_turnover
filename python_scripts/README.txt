...
# For outputting average RPM and RPKM across replicates from the output of featureCounts, only for reconciledPeaks with names containing a_#### or b_#### (for use in speciesPairs pipeline)
countRPM.py

# For outputting average RPM and RPKM across replicates from the output of featureCounts, this time for all peaks in the file (for use in singleSpecies pipeline)
countRPM_general.py

# For outputting average RPM and RPKM across replicates from the output of featureCounts, only for reconciledPeaks with names containing a_#### or b_#### (for use in speciesPairs pipeline)
# Note that here "RPM" and "RPKM" should not be interpreted literally, since I'm quantifying signal from bigWigs instead of raw reads for the Noonan datasets
countBigWigSignal.py
